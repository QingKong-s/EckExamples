...
{
	// TODO: 更新固定大小的控件
	m_BT.Position = POINT{ eck::DpiScale(10, m_iDpi), eck::DpiScale(10, m_iDpi) };
	m_BT.SetSize({ m_Ds.cx,m_Ds.cy });
}
...
void OutputRange(eck::CLVRange* psr)
{
	EckDbgPrint("{");
	for (const auto& e : psr->GetSelRange().GetList())
		EckDbgPrintFmt(L"[%d, %d]", e.idxBegin, e.idxEnd);
	EckDbgPrint("}");
}
...
BOOL CWndMain::OnCreate(HWND hWnd, CREATESTRUCT* lpCreateStruct)
{
	...
	const auto pstd = LVRange_Create();
	const auto psr = new eck::CLVRange{};
	EckDbgPrint(L"===IncludeRange===");
	psr->IncludeRange(5, 10);
	OutputRange(psr);
	psr->IncludeRange(15, 20);
	OutputRange(psr);
	psr->IncludeRange(25, 30);
	//psr->IncludeRange(1, 9999999);
	//OutputRange(psr);

	pstd->IncludeRange(5, 10);
	pstd->IncludeRange(15, 20);
	pstd->IncludeRange(25, 30);
	CheckRangeEqual(pstd, psr, 50);

	EckDbgPrint(L"===ExcludeRange===");
	psr->Clear();
	psr->IncludeRange(5, 10);
	OutputRange(psr);
	psr->ExcludeRange(6, 10);
	OutputRange(psr);

	pstd->Clear();
	pstd->IncludeRange(5, 10);
	pstd->ExcludeRange(6, 10);
	CheckRangeEqual(pstd, psr, 50);
	...
	psr->IncludeRange(5, 10);
	OutputRange(psr);
	psr->ExcludeRange(5, 9);
	OutputRange(psr);
	EckDbgPrint(L"===");
	psr->Clear();
	psr->IncludeRange(5, 10);
	OutputRange(psr);
	psr->ExcludeRange(5, 10);
	OutputRange(psr);
	EckDbgPrint(L"===");
	psr->Clear();
	psr->IncludeRange(5, 10);
	OutputRange(psr);
	psr->ExcludeRange(6, 9);
	OutputRange(psr);
	EckDbgPrint(L"===");
	psr->Clear();
	psr->IncludeRange(5, 10);
	...
	psr->IncludeRange(25, 30);
	OutputRange(psr);
	psr->ExcludeRange(12, 22);
	OutputRange(psr);
	EckDbgPrint(L"===InvertRange===");
	psr->Clear();
	psr->IncludeRange(5, 10);
	psr->IncludeRange(15, 20);
	psr->IncludeRange(25, 30);
	OutputRange(psr);
	psr->InvertRange(8, 26);
	OutputRange(psr);
	psr->InvertRange(8, 26);
	OutputRange(psr);
	...
	OutputRange(psr);
	LONG t;
	psr->NextSelected(10, &t);
	EckDbgPrint(t);
	psr->NextSelected(15, &t);
	EckDbgPrint(t);
	EckDbgPrint(L"===");
	psr->Clear();
	psr->IncludeRange(0, 10);
	psr->IncludeRange(15, 999);

	pstd->Clear();
	pstd->IncludeRange(0, 10);
	pstd->IncludeRange(15, 999);
	CheckRangeEqual(pstd, psr, 9999);
	EckDbgPrint(L"===");
	psr->Clear();
	psr->IncludeRange(0, 0);
	psr->IncludeRange(2, 5);
	psr->IncludeRange(1, 1);
